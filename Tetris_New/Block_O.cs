...
namespace Tetris_New {
    public class Block_O : Block {
        ...
        public override bool canMoveLeft(Map map) {
            ...
            if (!p1.isValid() || !p2.isValid()) return false;               //撞墙
            if (map.getValue(p1) != 0 || map.getValue(p2) != 0) return false;       //有障碍
            return true;
        }
        ...
        public override bool canMoveDown(Map map) {
            Point p1 = new Point(point2.X + 1, point2.Y);
            Point p2 = new Point(point3.X + 1, point3.Y);
